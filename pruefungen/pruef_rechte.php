...
pruef('Es gibt genau die vier entschiedenen Rollen', count($katalog) === 4);
pruef('Die vier Rollen heissen wie entschieden',
    array_keys($katalog) === ['mitarbeitend', 'planung', 'personal', 'verwaltung']);
pruef('Jede Rolle hat Titel und erklaerenden Text',
    count(array_filter($katalog, fn($r) => $r['titel'] !== '' && strlen($r['text']) > 30)) === 4);
...
pruef('KRITISCH: Verwaltung darf alles',
    count(array_diff($alleRechte, rechte_aus_rollen(['verwaltung']))) === 0);
